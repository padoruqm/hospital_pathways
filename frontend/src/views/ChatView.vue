  ...
  <section class="chat">
    <header class="chat-head">
      ...
      <div class="mode" role="group" aria-label="Chế độ AI">
        <button :class="{ on: mode === 'rag' }" @click="mode = 'rag'">
          RAG (tra tài liệu)
        </button>
        <button :class="{ on: mode === 'system' }" @click="mode = 'system'">
          Hỏi nhanh
        </button>
      </div>
    </header>
        ...
        <div class="bubble">
          ...
          <div v-if="m.sources && m.sources.length" class="sources">
            ...
            <template v-for="(s, si) in m.sources" :key="s.id || s.name">
              ...
              ><span v-if="si < m.sources.length - 1"> · </span>
            </template>
          </div>
        </div>
      </div>
    ...
    <form class="composer" @submit.prevent="send(input)">
      <input
        ...
        :disabled="sending"
      />
      <button type="submit" class="send" :disabled="sending || !input.trim()">
        Gửi
      </button>
    </form>
  </section>
